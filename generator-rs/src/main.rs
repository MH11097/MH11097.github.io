...
fn generate_index_page(posts: &Vec<Metadata>) {
    ...
    if let Ok(template) = load_template("index") {
        let html: Vec<String> = posts.into_iter().map(|p| {
            ...
            let post_date = NaiveDateTime::parse_from_str(&p.date, TIME_FORMAT).unwrap().and_utc();
    ...
    if args.len() > 1 {
        ...
        if param != "preview" {
            ...
            if let Ok(template) = load_template(folder) {
                ...
                posts.sort_by(|a, b| {
                    let ta = NaiveDateTime::parse_from_str(&a.date, TIME_FORMAT).unwrap().and_utc();
                    let tb = NaiveDateTime::parse_from_str(&b.date, TIME_FORMAT).unwrap().and_utc();
                    tb.cmp(&ta)
                });
